...
while running:
    ...
    color = ["black", "r", "g","r", "g", "b", "b", "b", "b", "b"]
    fig, (ax[0],ax[1], ax[2], ax[3], ax[4]) = plt.subplots(5,1,figsize=(6,6))
    for i in range(len(ax)):
        ...
        line[i + 5], = ax[i].plot(xx, yy[i], color[i + 5])
        line[i + 5].set_linewidth(2)
    ...
    while not gameover:
        ...
        yy[5].append(prodeaths)
        yy[6].append(pronewinfection)
        yy[7].append(prodeaths)
        yy[8].append(proinfections)
        yy[9].append(proimmun)
        for i in range(len(ax)):
            if len(proxx) + protime > xrange:
               ax[i].set_xlim(proxx[protime - xrange], proxx[-1])
            ax[i].set_ylim(0, max([max(yy[i + 5]), max(yy[i])]) * 1.5)
            line[i].set_xdata(xx)
            line[i].set_ydata(yy[i])
            line[i + 5].set_xdata(proxx)
            line[i + 5].set_ydata(yy[i + 5])
        #plt.draw()
        #plt.pause(0.001)
        ...
        if days > protime:
            textgrund,textkasten = textobjekt(str(round(yy[8][-protime] - allinfections)),font)
            textkasten.center = (width - 200, 100)
            screen.blit(textgrund, textkasten)
            totaldifference += yy[8][-protime] - allinfections
            yy[6][-protime] = newinfection
        ...
        prodeaths = deathrate / 100 * newinfection
        
        print(8 - (population / resolution**2 * 8) - (allimmun / population * 8) - (deathcounter / population * 8) - (allinfections / population * 8))
        if days > duration:
            pronewinfection = yy[6][-1] + yy[6][-1] * (infectodds / 100) * (8 - (population / resolution**2 * 8) - (allimmun / population * 8) - (deathcounter / population * 8) - (allinfections / population * 8)) - yy[6][-duration] * randint(7, 8)
        else:
            pronewinfection = yy[6][-1] + yy[6][-1]**10 * (infectodds / 100) * 8 * allimmun / population# * deathcounter / population * allinfections / population
        if pronewinfection < 2:
            pronewinfection = 2
        
        proimmun = allimmun + newinfection  * ((100 - deathrate) / 100) - newimune
        if vaccineresearch < protime:
            proimmun += vaccineproduktion
        
        if days > duration:
            proinfections = allinfections - yy[6][-duration]
        else:
            proinfections = allinfections
    ...
    while not finish:
        ...
        textgrund,textkasten = textobjekt(str(round(totaldifference / days)),font)
        textkasten.center = (width - 200, 100)
        screen.blit(textgrund, textkasten)
